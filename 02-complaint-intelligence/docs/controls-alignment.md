...
| Policies applied consistently across workloads                    | Guardrail policies defined at platform level, consumed by workloads                                            | The same policy definitions used by a companion control-plane demonstration fire identically against this workload                                                                       | Shared policy definition; matching guardrail decisions across both contexts                      |
...
- **Span structure.** All tracing follows the four-stage span convention
  (application, input/guardrails, retrieval, model) so evidence from this
  workload is structurally identical to evidence from a companion control-plane
  demonstration.
