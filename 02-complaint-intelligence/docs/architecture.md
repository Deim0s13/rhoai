...
| Vector store      | Vector store in the knowledge layer                                    | Inline Milvus, on the mounted PVC (survives pod restarts)                               | Matches the conceptual architecture and the 2.25 build; consistent with existing customer-facing material. RHOAI 3.4 registers no vector_io provider by default; this is opt-in. See ADR-0008               |
| Embedding model   | Implicit in the knowledge layer                                        | Inline sentence-transformers, CPU-based (nomic-embed-text-v1.5, 768 dims)               | Avoids a second GPU consumer on the single-L4 cluster. RHOAI 3.4 registers no embedding model by default; read at runtime, not hardcoded, since the model itself changed between 2.25 and 3.4. See ADR-0008 |
...
1. **Gateway composition (ADR-0003, still Draft).** MaaS is present as Gateway
...
2. **Second model backend.** Owed for the policy-consistency proof (ADR-0001)
